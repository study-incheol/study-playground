...
- 이벤트 소싱과 코레오그래피 사가는 찰떡궁합이나
- 물론 코레오그래피 사가는 단순해서 좋지만 단점도 많다
- 사가 코레오그래피에 이벤트를 사용하면 이벤트의 목적이 이원화된다
- 이벤트 소싱은 상태 변화를 나타내기 위해 이벤트를 이용하는데, 이벤트를 사가 코레오그래피에 갖다 쓰면 애그리거트에 갖다 쓰면 애그리거트는 상태 변화가 없어도 무조건 이벤트를 발생시켜야 한다
...
- 서비스는 이 두가지 액션을 첫 번째 액션이 수행되면 두 번째 액션은 최종적으로 실행되는 방식으로 수행한다
