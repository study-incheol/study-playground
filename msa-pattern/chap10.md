...
### 🤫 중간에 운영환경 소스코드를 재배포 해야 하거나 다시 빌드해야 하는 상황이 생기면 어떻게 될까?

- 운영환경의 소스코드는 아직 개발환경의 소스코드가 모두 반영되지 않았으므로 테스트는 실패할것이다
- 그러면 우리는 두 가지 방법 중에 하나를 선택할 것이다
  - 하나는 실패하는 테스트 코드를 스킵(Ignore) 하는것이다
  - 또 하나는 실패하는 테스트 코드만 수정하는 것이다
- 두 방법 모두 만족할 만한 해결책은 아닌것 같다.. 😱

### 결국 독립적인 테스트 환경의 데이터베이스가 필요하다
